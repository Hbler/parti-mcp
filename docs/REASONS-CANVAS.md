...
- **Geometry**: Turf.js does all spatial math at both scales — bbox of the full spec, coordinate scaling into an SVG `viewBox`, `turf.buffer` to turn centerlines (roads *or* walls) into polygons, `turf.centroid` for auto-placed labels.
...
**Status: draft — pending your review before Operations 1–12 are implemented.**
