...
Capability boundary: plans are a 2D cut with a walls+rooms+openings+stairs+ladders+columns vocabulary (walls may be straight or circular-arc; heightClass full|low) and NO continuous vertical model. Not supported: furniture/fixtures, MEP, and non-circular (spline/elliptical) curves. Mixed wall materials on one floor ARE supported (rendered per material group).
